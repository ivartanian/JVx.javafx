...
public class FXCheckBoxTestMain extends Application
{
	...
	{
		FXCheckBox checkBox = new FXCheckBox("Some text", new ImageView(new Image("http://upload.wikimedia.org/wikipedia/commons/6/6f/Icon-designed_using_vim.png")));
		checkBox.setStyle("-fx-background-color: #CCCCCC;");
		
		ComboBox<ContentDisplay> contentBox = new ComboBox<>();
		contentBox.getItems().addAll(ContentDisplay.values());
		contentBox.setValue(checkBox.getContentDisplay());
		checkBox.contentDisplayProperty().bind(contentBox.valueProperty());
		
		ComboBox<HPos> hPosBox = new ComboBox<>();
		hPosBox.getItems().addAll(HPos.values());
		hPosBox.setValue(checkBox.getRelativeHorizontalTextAlignment());
		checkBox.relativeHorizontalTextAlignment().bind(hPosBox.valueProperty());
		
		ComboBox<VPos> vPosBox = new ComboBox<>();
		vPosBox.getItems().addAll(VPos.values());
		vPosBox.setValue(checkBox.getRelativeVerticalTextAlignment());
		checkBox.relativeVerticalTextAlignment().bind(vPosBox.valueProperty());
		...
		imageCheckBox.setOnAction((pActionEvent) ->
		{
			...
			{
				checkBox.setGraphic(new ImageView(new Image("http://upload.wikimedia.org/wikipedia/commons/6/6f/Icon-designed_using_vim.png")));
			}
				else
				{
		...
		primaryStage.setTitle("JavaFX FXCheckBox Test");
		primaryStage.show();
	}
}
